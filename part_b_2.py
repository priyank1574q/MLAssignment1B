...
def batchmaker(train_data, y_train):
    x_cross = []
    y_cross = []
    for i in range(0,train_data.shape[0],128):
        x_cross.append(train_data[i:i+128,:])
    for i in range(0,y_train.shape[0],128):
        y_cross.append(y_train[i:i+128,:])
...
def logistic(train_data, y_train, lamda, rate):
    ...
    x, y = batchmaker(train_data, y_train)
    for i in range(len(x)):
        for j in range(5):
            w1 = w0 - (rate/(j+1)**(0.5))*gradient(x[i], y[i], w0, lamda)
...
#lamda = [10**(-2), 10**(-1), 10**(0), 2]
#for i in range(len(lamda)):
#    print(str(lamda[i]) + '---------' + str(cross_acc(train_data,  y_train, lamda[i], 0.3)))
