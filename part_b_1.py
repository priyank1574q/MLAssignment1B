...
def batchmaker(train_data, y_train):
    ...
    for i in range(0,train_data.shape[0],batch_size):
        x_cross.append(train_data[i:i+batch_size,:])
    for i in range(0,y_train.shape[0],batch_size):
        y_cross.append(y_train[i:i+batch_size,:])
...
def logistic(train_data, y_train, lamda, rate):
    ...
    for i in range(len(x)):
        for j in range(5):
            w1 = w0 - rate*gradient(x[i], y[i], w0, lamda)
...
#lamda = [10**(-1), 10**(0), 10, 10**(2), 10**(3)]
#for i in range(len(lamda)):
#    print(str(lamda[i]) + '---------' + str(cross_acc(train_data,  y_train, lamda[i], 0.1)))
